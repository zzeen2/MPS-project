...
exports.buildMusicTrendDailyQuery = buildMusicTrendDailyQuery;
exports.buildMusicTrendMonthlyQuery = buildMusicTrendMonthlyQuery;
const drizzle_orm_1 = require("drizzle-orm");
const useCaseFilter = (type) => type === 'music' ? (0, drizzle_orm_1.sql) `mp.use_case IN ('0','1')` : (0, drizzle_orm_1.sql) `mp.use_case = '2'`;
function buildMusicTrendDailyQuery(params) {
...
plays_by_music AS (
  ...
  FROM music_plays mp, month_range mr
  ${segment === 'category' ? (0, drizzle_orm_1.sql) `JOIN musics m ON m.id = mp.music_id` : (0, drizzle_orm_1.sql) ``}
  WHERE mp.is_valid_play = true
    AND ${useCaseFilter(type)}
    AND mp.created_at >= mr.month_start AND mp.created_at <= mr.month_end
    ${segment === 'category' ? (0, drizzle_orm_1.sql) `AND m.category_id = (SELECT category_id FROM base)` : (0, drizzle_orm_1.sql) ``}
  GROUP BY 1,2
),
industry_avg AS (
...
function buildMusicTrendMonthlyQuery(params) {
...
plays_by_music AS (
  ...
  LEFT JOIN music_plays mp ON mp.is_valid_play = true
    AND ${useCaseFilter(type)}
    AND mp.created_at >= sb.month_start AND mp.created_at <= sb.month_end
  ${segment === 'category' ? (0, drizzle_orm_1.sql) `LEFT JOIN musics m ON m.id = mp.music_id` : (0, drizzle_orm_1.sql) ``}
  ${segment === 'category' ? (0, drizzle_orm_1.sql) `WHERE (m.category_id = (SELECT category_id FROM base) OR mp.music_id IS NULL)` : (0, drizzle_orm_1.sql) ``}
...
//# sourceMappingURL=trend.queries.js.map