...
exports.buildSummaryListBaseQuery = exports.buildMonthlyIndustryAvgQuery = exports.buildMonthlyCompanyQuery = exports.buildByMusicQuery = exports.buildDailyIndustryAvgQuery = exports.buildDailyQuery = exports.buildSummaryQuery = void 0;
const drizzle_orm_1 = require("drizzle-orm");
const buildSummaryQuery = (companyId, ymYear, ymMonth, tz) => (0, drizzle_orm_1.sql) `
  WITH month_range AS (
    SELECT 
  ...
  LIMIT 1;
`;
exports.buildSummaryQuery = buildSummaryQuery;
const buildDailyQuery = (companyId, ymYear, ymMonth, tz) => (0, drizzle_orm_1.sql) `
  WITH month_range AS (
    SELECT 
      make_timestamptz(${ymYear}, ${ymMonth}, 1, 0, 0, 0, ${tz}) AS month_start,
      (make_timestamptz(${ymYear}, ${ymMonth}, 1, 0, 0, 0, ${tz}) + interval '1 month') - interval '1 second' AS month_end
  ...
  ORDER BY days.d ASC;
`;
exports.buildDailyQuery = buildDailyQuery;
const buildDailyIndustryAvgQuery = (ymYear, ymMonth, tz) => (0, drizzle_orm_1.sql) `
  WITH month_range AS (
    SELECT 
      make_timestamptz(${ymYear}, ${ymMonth}, 1, 0, 0, 0, ${tz}) AS month_start,
      (make_timestamptz(${ymYear}, ${ymMonth}, 1, 0, 0, 0, ${tz}) + interval '1 month') - interval '1 second' AS month_end
  ...
  ORDER BY days.d ASC;
`;
exports.buildDailyIndustryAvgQuery = buildDailyIndustryAvgQuery;
const buildByMusicQuery = (companyId, ymYear, ymMonth, tz) => (0, drizzle_orm_1.sql) `
  WITH month_range AS (
    SELECT 
      make_timestamptz(${ymYear}, ${ymMonth}, 1, 0, 0, 0, ${tz}) AS month_start,
      (make_timestamptz(${ymYear}, ${ymMonth}, 1, 0, 0, 0, ${tz}) + interval '1 month') - interval '1 second' AS month_end
  ...
  LIMIT 100;
`;
exports.buildByMusicQuery = buildByMusicQuery;
const buildMonthlyCompanyQuery = (companyId, ymYear, ymMonth, months, tz) => (0, drizzle_orm_1.sql) `
  WITH anchor AS (
...
`;
exports.buildMonthlyCompanyQuery = buildMonthlyCompanyQuery;
const buildMonthlyIndustryAvgQuery = (ymYear, ymMonth, months, tz) => (0, drizzle_orm_1.sql) `
  WITH anchor AS (
    SELECT make_timestamptz(${ymYear}, ${ymMonth}, 1, 0, 0, 0, ${tz}) AS anchor_month
  ),
  month_series AS (
  ...
  ORDER BY ms.m_start ASC;
`;
exports.buildMonthlyIndustryAvgQuery = buildMonthlyIndustryAvgQuery;
const buildSummaryListBaseQuery = (ymYear, ymMonth, tz) => (0, drizzle_orm_1.sql) `
  WITH month_range AS (
    SELECT 
      make_timestamptz(${ymYear}, ${ymMonth}, 1, 0, 0, 0, ${tz}) AS month_start,
      (make_timestamptz(${ymYear}, ${ymMonth}, 1, 0, 0, 0, ${tz}) + interval '1 month') - interval '1 second' AS month_end
  ...
  WHERE 1=1
`;
exports.buildSummaryListBaseQuery = buildSummaryListBaseQuery;
