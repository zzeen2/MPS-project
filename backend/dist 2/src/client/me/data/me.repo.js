...
class MeRepo {
    ...
    async getMonthLabel() {
        const { rows } = await this.pool.query(`SELECT to_char(timezone($1, now()), 'YYYY-MM') AS ym`, [TZ]);
        return rows[0]?.ym ?? '';
    }
    async listCompanyMusics(companyId, musicId) {
    ...
    async getAggregates(companyId, musicId) {
        const { rows } = await this.pool.query(`
      WITH month_spent AS (
      ...
      `, [companyId, musicId, me_rewards_dto_1.REWARD_CODE_EARNING, TZ]);
        return rows[0];
    }
    async getDaily(companyId, musicId, days) {
        const { rows } = await this.pool.query(`
      WITH days AS (
        SELECT dd::date AS d
      ...
      GROUP BY d
      ORDER BY d
      `, [companyId, musicId, TZ, days, me_rewards_dto_1.REWARD_CODE_EARNING]);
        return rows;
    }
    async countPlays(companyId, musicId) {
        const { rows } = await this.pool.query(`SELECT COUNT(*)::text AS c FROM music_plays WHERE company_id=$1 AND music_id=$2`, [companyId, musicId]);
        return Number(rows[0]?.c ?? '0');
    }
    async listPlays(companyId, musicId, limit, offset) {
        const { rows } = await this.pool.query(`
      ...
      LIMIT $4 OFFSET $5
      `, [companyId, musicId, me_rewards_dto_1.REWARD_CODE_EARNING, limit, offset]);
        return rows;
    }
}
exports.MeRepo = MeRepo;
//# sourceMappingURL=me.repo.js.map